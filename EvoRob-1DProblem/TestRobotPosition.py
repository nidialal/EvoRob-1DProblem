...
conditions = [0.224,0.226,0.228]

for c in conditions:

	init[-1] = c
	set_params(0.42296)
	desc = "RobotInit_1_"+str(c)
	nar_p = True
	wide_p = True
	x_rob = []
	t_rob = []
	...
	'''
	plt.figure("Combined",figsize=(12.0, 6.0))
	plt.scatter(t_rob,[x%1 for x in x_rob],s=0.1)
...
plt.subplots_adjust(bottom=.25, left=.25)
plt.plot([fn_s(x,xdiff,nar_p,wide_p)*40 for x in x_sensor],x_sensor,color='#ca3587')
plt.savefig(folderName+"/RobMotion_Combined_Initial_1"+".eps")
plt.show()
