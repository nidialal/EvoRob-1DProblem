...
near_axis = 0
dtTime = datetime.now().strftime("%Y-%m-%d--%H:%M:%S")+".txt"
fNameInfo = "fitness_Init_"+dtTime
fit_file = open(fNameInfo,"a+")
fit_file.write(str(init)+"\n")
fit_file.write(str(near_axis))
fit_file.close()

fName = "fitness_"+ dtTime
...
for i in xdist:
	...
	for j in xinit:
		...
		sol = odeint(model,init,t,full_output=True)
		nodes = sol[0]
