...
def set_params():
	global xdiff
	xdiff = 0.42296#random.uniform(-0.3,0.3)
	st_pos = (8.0/9) +xdiff
...
for i in range(numNodes):
	for j in range(numNodes):
		...
		if(adjMat[i][j]==1):
			...
			edgeList.append((i,j))
			init.append(random.uniform(0.01,0.11))
init.append(0.218)#random.random())
...
near_axis = int(lines[1])

fitFile.close()

init=[float(num) for num in lines[0].split(","))
...
if(op.isfile(folderName+"/counter.txt")):
	...
	counter = 1
'''
counter = "-" if (xdiff<0) else "" + "0_"+str(round(abs(xdiff)*100,2))
...
ax3.plot([fn_s(x,xdiff)*2 for x in x_sensor],x_sensor,color='#ca3587')
...
plt.savefig(folderName+"/Analysis"+str(counter)+".pdf")

plt.figure(figsize=(10,2))
plt.scatter(t_rob,[x%1 for x in x_rob],s=0.1,color='#3fb7e9')
x_sensor = np.linspace(0,1,200)
plt.plot([fn_s(x,xdiff)*2 for x in x_sensor],x_sensor,color='#ca3587')
plt.xlabel("Integration time/Sensor Value")
plt.ylabel("Robot Position")
plt.subplots_adjust(bottom=.25, left=.25)
plt.savefig(folderName+"/RobMotion_"+str(counter)+".pdf")
...
plt.ylabel("Robot Position")
plt.subplots_adjust(bottom=.25, left=.25)
plt.savefig(folderName+"/RobMotion_Zoomed_"+str(counter)+".pdf")
'''
