...
conditions = np.linspace(0.45,0.75,31)
...
for c in conditions:
	...
	'''
	plt.figure("Analysis"+str(i),figsize=(12.0, 6.0))
	...
	plt.savefig(folderName+"/Analysis_"+desc+".png")
	...
	plt.savefig(folderName+"/RobMotion_"+desc+".png")
	...
	plt.ylabel("Robot Position")
	plt.subplots_adjust(bottom=.25, left=.25)
	plt.savefig(folderName+"/RobMotion_Zoomed_"+desc+".png")
	'''
	plt.figure("Combined",figsize=(14.0, 8.0))
	plt.scatter(t_rob,[x%1 for x in x_rob],s=0.1,label="Init: "+str(c))

x_sensor = np.linspace(0,1,200)
plt.xlabel("Integration time/Sensor Value")
plt.ylabel("Robot Position")
plt.subplots_adjust(bottom=.25, left=.25)
plt.plot([fn_s(x,xdiff,nar_p,wide_p)*20 for x in x_sensor],x_sensor,color='#ca3587')
#plt.savefig(folderName+"/RobMotion_Combined_Initial_"+desc+".eps")
plt.show()
