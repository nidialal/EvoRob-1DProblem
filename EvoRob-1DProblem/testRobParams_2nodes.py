...
def model(t,init):
	...
	for i,edge in enumerate(edgeList):
		ydot = g(y_vals[i], 
			A - (beta[i] * np.square(p_vals[edge[0]])) + (C * (y_squared - np.square(y_vals[i])))) + (gamma[i]*fn_s(x_val%1,np_pos,xdiff))
...
#Analysis 4
alpha=[0.0065342750516958724, -0.00051364600489010696]
beta=[1.427562136885423, 1.547501541040194]
gamma=[0.486844848131448, 0.094738639550160514]
...
y_index = []
x_cur = 0
r = ode(model).set_integrator('vode',atol=1e-12)
...
plt.subplot(414)
plt.scatter(t_rob,[fn_s(x%1,np_pos,xdiff) for x in x_rob],s=0.1,label="Sensor value")
plt.legend(loc="upper right")

plt.figure("Sensor_")
x1 = np.linspace(0,1,1000)
plt.plot([fn_s(x%1,np_pos,xdiff) for x in x1],x1)
